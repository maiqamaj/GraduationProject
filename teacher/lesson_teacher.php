...
date_default_timezone_set('Asia/Amman');
		...
		if($file_size < 100000000000000000){
			...
			if(in_array($end, $allowed_ext)){
				...
				if(move_uploaded_file($file_temp, $location)){
					mysqli_query($con, "INSERT INTO `lesson`(teacher,class,subject,video_name ,location,details,creationDate) VALUES('$t','$class','$subject','$name', '$location','$details','$n')") or die(mysqli_error());
	...
	if(isset($_GET['del']))
      {     $de="DELETE FROM lesson where id = '".$_GET['id']."'";
            $rt=mysqli_query($con,$de);
       if($rt)
       {  $_SESSION['dm']="تم حذف الدرس بنجاح";}
      }
        ...
        <div class="container">
              <div class="row">                       
                    <div class="col-md-12">
                        ...
                        <h1 class="page-head-line"> الدروس  </h1>
...
<?php echo htmlentities($_SESSION['msg']);?><?php echo htmlentities($_SESSION['msg']="");?></font>
   ...
   ?>
       <option value="" disabled selected hidden>اختر الصف \الشعبه </option>	
                                  ...
                                  <thead>
                                    <tbody>
                                     ...
                                     <?php     $cnt=1; 
                                         ...
                                         $sql=mysqli_query($con,"SELECT DISTINCT * FROM `lesson` where class='".$da['class']."' 
                                       ...
                                       while( $row=mysqli_fetch_array($sql))
                                       {
                                            ...
                                            $x=substr($row['creationDate'], 0, 11);
   $x1=substr($row['creationDate'], 11);
                                        ?>
                                        <tr>
										...
										<td style="text-align:center"><video width="40%" height="100" controls>
									 ...
									 <td style="text-align:center"><?php echo 'التاريخ : ',$x,'</br>','الساعة :',$x1; ?></td>
										  ...
										  <a href="showlesson.php?id=<?php echo $row['id']?>">
										  <button class="btn btn-primary" style="background-color:#008000;"> عرض</button> </a>  
                                                  
                                                                                   
										  <a href="lesson_teacher.php?id=<?php echo $row['id']?>&del=delete" onClick="return confirm(' متأكد من حذف الدرس')">
