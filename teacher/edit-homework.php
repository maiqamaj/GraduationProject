...
if(isset($_POST['submit']))
{
 ...
 if($vat){            
$ret=mysqli_query($con,"update homework set class='$class',subject='$subject',start_time='$start_time',end_time='$end_time',details='$details',sumbitbate='$n' where id='$id'");
     
  $cl= mysqli_query($con,"select id_note from notification where id_note='$id'");
   $clasq= mysqli_fetch_array($cl);
     if($clasq==0)
     { $t=$_SESSION['login'];
     $ss="INSERT INTO notification(teacher,num_notification,id_note,class,subject,currentdate) VALUES('$t','1','$id','$class','$subject','$n')";
      $rr=mysqli_query($con,$ss);}
     
    else {$ttt= $clasq[0];
    $rrr=mysqli_query($con,"update notification set class='$class',subject='$subject',num_notification='1',currentdate='$n' where id_note='$id'");
     }
if($ret)
{
 ...
 <div class="form-group" align="right">
    ...
    <input type="text" type="datetime-local" class="form-control" id="end_time" name="end_time" placeholder="" value="<?php   echo $row['end_time'];   ?>" required />
     ...
     <div  class="form-group" align="right" >
         ...
         <input  type="text" id="details" name="details"class="form"  value="<?php echo htmlentities($row['details']);?>" /></div>  
  
         <div class="form-group" align="right">
    <label for="end_time" align="right">وقت اضافه الواجب</label>
    <input type="text" type="datetime-local" class="form-control"  placeholder="" value="<?php $XX=substr($row['sumbitbate'], 0, 11);
   $XX1=substr($row['sumbitbate'], 11);  echo 'التاريخ :',$XX,'&nbsp;&nbsp;','الساعة :',$XX1;?>" readonly/>
  </div>   
