...
{
$sql=mysqli_query($con,"SELECT password FROM  student where password='".md5($_POST['cpass'])."' && id='".$_SESSION['login']."'") or die(mysqli_error($con));
...
if($uppercase &&  $lowercase && $number && strlen($password) >= 8 && $symbol && !$space) {
 ...
 $concat=mysqli_query($con,"update student set password='".md5($_POST['newpass'])."', updationDate='$currentTime' where id='".$_SESSION['login']."'");
