 ...
 <a href="home.php"> <b> <button  style="position: absolute; 
  left: 50px; top:30px; color:#6F479F; border: none; background-color:#e8e8e8;">العوده</button></b></a>	 
   ...
   <main>
     ...
     <center><div style=" background-color: #fafafa;  max-width:800px; max-height=1000px;" >  
...
$sql=mysqli_query($con,"SELECT DISTINCT subject FROM `lesson` where class='".$class['section_id']."' ");
while(  $row=mysqli_fetch_array($sql))
{
        

  $subject1=$row['subject'];
  $s1= mysqli_query($con,"select * from subject where id = '$subject1'");
  ...
  ?>
    ...
    align-content: space-between;  width:170px; height: 200px; ">
                <a href= "showlesson_student.php?id=<?php echo $subject['id']?>"  style="text-decoration:none;" >
                <?php  
                    if($subject['subjectName']=='الرياضيات' or $subject['subjectName']=='الماليه')
                       ...
                       ?>
     <button  class="button"  style="  width:170px; height: 150px; background-image: url(<?php  echo $xx; ?>); background-repeat: no-repeat;
  background-size: 100% 100%; ">
        
          
                   </button><?php  echo "<h5 style='color:#6F479F'><b>",$subject['subjectName'],'</b></h5>';?> 
