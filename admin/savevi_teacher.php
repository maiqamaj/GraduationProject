<?php
		...
		if($file_size < 100000000000000000){
			...
			if(in_array($end, $allowed_ext)){
				...
				if(move_uploaded_file($file_temp, $location)){
					mysqli_query($con, "INSERT INTO `videotea` VALUES('$id' , '$name', '$location')") or die(mysqli_error());
					echo "<script>alert('تم اضافة الفيديو ')</script>";
					echo "<script>window.location = 'leasonteach.php'</script>";
				}
			}else{
	...
	if(ISSET($_GET['delete'])){
		$id = $_GET['delete'];
		mysqli_query($con, "DELETE FROM videotea WHERE video_id = $id ") or die(mysqli_error());
		echo "<script>window.location = 'leasonteach.php'</script>";
